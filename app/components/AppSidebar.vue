...
<template>
  ...
  <div class="flex flex-col h-full gap-0.5 py-3" :class="collapsed ? 'px-2.5' : 'px-3'">
    <div class="flex items-center h-11 shrink-0 mb-2" :class="collapsed ? 'justify-center px-0' : 'px-1'">
      <div v-if="!collapsed" class="flex items-center gap-2.5">
        ...
        <div>
          <span
            class="font-heading font-bold text-[15px] tracking-tight text-highlighted leading-none block">TaskFlow</span>
    ...
    <div class="space-y-0.5 mt-1">
      <div v-for="item in mainNav" :key="item.to">
        <UTooltip v-if="collapsed" :text="item.label" side="right" :delay="300">
          <NuxtLink v-slot="{ isActive: active }" :to="item.to" custom>
            <div
        ...
        <nuxt-link v-else v-slot="{ isActive: active }" :to="item.to" custom>
    ...
    <template v-if="!collapsed">
      <div class="mt-4">
        ...
        <div class="space-y-0.5">
          <NuxtLink v-for="ws in workspaces" :to="`/workspaces/${ws.id}`" v-slot="{ isActive: active }" custom :key="ws.id">
            <nuxt-link :to="`/workspaces/${ws.id}`"
              ...
              :class="active
                ...
                : 'text-muted hover:text-default hover:bg-white/60 dark:hover:bg-white/6'">
              <UIcon :name="ws.icono" class="w-4 h-4 shrink-0" :style="{backgroundColor: ws.color}" />
    ...
    <div class="mt-auto pt-3 border-t border-white/50 dark:border-white/10">
      <NuxtLink v-if="!collapsed" v-slot="{ isActive: active }" to="/settings" custom>
        <div
          ...
          <UButton to="/auth/login" icon="i-lucide-arrow-left-from-line" variant="ghost" color="error"></UButton>
        </div>
      </NuxtLink>
      <UTooltip v-else text="Perfil" side="right" :delay="300">
        <NuxtLink to="/settings" class="flex justify-center py-1">
          <div class="relative">
            <UAvatar src="https://api.dicebear.com/9.x/initials/svg?seed=CM&backgroundColor=2563eb&textColor=ffffff"
              size="xs" />
            <div
              class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-green-500 ring-2 ring-white dark:ring-bg" />
          </div>
